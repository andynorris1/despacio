...
// One row per city tab, with the city, year and event from GIGS above.
// Slug is the page address (sets/<slug>/). It's set once and then kept, so links
// people have shared keep working.
function rebuildAppearances_(tabs, years) {
  ...
  let slugCol = -1;
  if (sh.getLastRow() > 0) {
    slugCol = findCol_(sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0], ['slug']);
    rows_(sh).forEach(r => { prev[r[1]] = r; });
  }
  ...
  const rows = tabs.map(s => {
    ...
    const p = prev[s.getSheetId()];
    let slug = (p && slugCol >= 0 && String(p[slugCol]).trim()) || slugify_(`${city} ${year}`) || String(s.getSheetId());
...
// Searches only tracks that have no URL and no status. Skips Unknown tracks
