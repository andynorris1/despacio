...
function aboutPage(list) {
  const root = '../';
  const gigs = list.filter(a => a.year).length; // Despacio Classics isn't a gig
  return page({
...
<h2>Which gigs appear</h2>
<p>If you do not see a gig, it's because there are less than 25 tracks identified for that gig.
Once the community spreadsheet has reached a threshold of 25 identified tracks, the gig will
be added. Right now ${gigs} gig${gigs === 1 ? '' : 's'} ${gigs === 1 ? 'is' : 'are'} listed, newest first.</p>
